//Function that runs when u open the page
window.onload = () => {
...
//Define the  size of the canvas
  canvas.width = theCanvas.width; //Define the width
  canvas.height = theCanvas.height; //Define the height
...
//Allow us to manipulate the HTML elements
  const startbutton = document.getElementById('start-button');  //Defining the start from HTML
  const resetbutton = document.getElementById('restart-button'); //Defining the restart button from HTML
  ...
  const player_width = 50; //width of the player
  const player_height = 50; //height of the player
  ...
  //Make a player class
  class Player{
    //BHelp us to define the variable of the players
...
// Start Function that control the game
  ...
  //Prepare the HTML to render the Game -> Setting the scene for the game
...
//Prepare the HTML to render the Game Over
  ...
  function updateBallPosition(){
    if ((positionWidth) < (theCanvas.width-40)) { //If the ball position is less than 460px
      positionWidth += 10; //make the ball move to the right
      ...
      //this means that the position in greater than the size of the canvas and we need to reduce the position by -10
      reduceCounter -= 10; //move the ball towards the left
      ctx.drawImage(ball, reduceCounter, theCanvas.height/2, 30, 30);
      if (reduceCounter < 40){ //This means we are aproaching the otherside of the canvans
        positionWidth = reduceCounter // we need to switch Direction to the right side 
  ...
  document.onkeydown = (e) => {
    if (e.keyCode === 37) {
      //When the left button is pressed (minus 10) on x-axis hence the player moves to the left
      player_x_value -= 10;
      //redraw the player
      playerGirl.makePlayer
    }
    else if (e.keyCode === 39) {
      //When the right button is pressed (plus 10) on x-axis hence the player moves to the right
      player_x_value += 10;
      //redraw the player
      playerGirl.makePlayer
    }
    else if (e.keyCode === 38) {
      //When the down button is pressed (minus 10) on y-axis hence the player moves down
      player_y_value -= 10;
      //redraw the player
      playerGirl.makePlayer
    }
    else if (e.keyCode === 40) {
      //When the up button is pressed (plus 10) on y-axis hence the player moves up
      player_y_value += 10;
